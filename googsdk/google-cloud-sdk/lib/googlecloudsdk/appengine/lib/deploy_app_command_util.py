...
def CopyFilesToCodeBucket(modules, bucket):
  ...
  with file_utils.TemporaryDirectory() as staging_directory:
    ...
    # TODO(user) Suppress the output and write it to a log. If the command
    # failed, point the user to the log.
    rsync_exit_code = cloud_storage.Rsync(staging_directory, bucket)
    if rsync_exit_code:
      raise exceptions.ToolException(('Could not synchronize files. The '
                                      'gsutil command exited with status '
                                      '[{s}].').format(s=rsync_exit_code))
...
def _BuildStagingDirectory(source_dir, staging_dir, bucket, excluded_regexes):
  ...
  for relative_path in util.FileIterator(source_dir, excluded_regexes,
    ...
    target_path = os.path.join(staging_dir, sha1_hash)
    if not os.path.exists(target_path):
      _CopyOrSymlink(local_path, target_path)

    dest_path = '/'.join([bucket_url.rstrip('/'), sha1_hash])
