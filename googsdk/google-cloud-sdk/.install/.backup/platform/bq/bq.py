...
def _CreateExternalTableDefinition(source_format, source_uris, schema):
  ...
  Arguments:
    source_format: Format of source data, case-insensitive. For CSV files,
      specify 'CSV'. For newline-delimited JSON, specify 'JSON' or
      'NEWLINE_DELIMITED_JSON'.
  ...
  try:
    ...
    if source_format.lower() not in supported_formats:
      raise app.UsageError(('%s is not a supported format.') % source_format)
    external_table_def['sourceFormat'] = (
        supported_formats[source_format.lower()])
...
class _MakeExternalTableDefinition(BigqueryCmd):
  usage = """mkdef <source_format> <source_uri> <schema>"""

  def __init__(self, name, fv):
    super(_MakeExternalTableDefinition, self).__init__(name, fv)

  def RunWithArgs(self, source_format, source_uris, schema):
    ...
    Usage:
      mkdef <source_format> <source_uris> <schema>

    Examples:
      bq mkdef CSV 'gs://bucket/file.csv' field1:integer,field2:string
    ...
    Arguments:
      ...
      schema: The <schema> argument should be either the name of a JSON file or
        a text schema. This schema should be omitted if the table already has
        one.
        ...
        text schema.
    """
    json.dump(_CreateExternalTableDefinition(source_format,
                                             source_uris,
                                             schema),
              sys.stdout)
...
class _Query(BigqueryCmd):
  ...
  def __init__(self, name, fv):
    ...
    flags.DEFINE_multistring(
        ...
        '"table_name::schema@format=uri". "format=" can be omitted for CSV '
        'tables.'
        'For example, '
...
class _Make(BigqueryCmd):
  ...
  def __init__(self, name, fv):
    ...
    flags.DEFINE_string(
        ...
        'The format of inline definition is "schema@format=uri". "format=" can'
        'be omitted for CSV tables.',
        flag_values=fv)
    flags.DEFINE_string(
        'view', '',
