...
def AutoscalersForMigs(mig_names, autoscalers, project, zone):
  """Finds Autoscalers with target amongst given IGMs.

  Args:
    mig_names: Target IGM names.
  ...
  igm_url_regex = re.compile(
      '/projects/{project}/zones/{zone}/instanceGroupManagers/({names})'.format(
          project=project,
          zone=zone,
          names=')|('.join(mig_names),
      )
  )
  result = [
...
def AutoscalerForMig(mig_name, autoscalers, project, zone):
  ...
  """
  autoscalers = AutoscalersForMigs((mig_name,), autoscalers, project, zone)
  if autoscalers:
