...
from googlecloudsdk.appengine.lib.external.api import namespace_manager
...
from googlecloudsdk.appengine.lib.external.datastore import datastore_pbs
from googlecloudsdk.appengine.lib.external.datastore import entity_v4_pb
from googlecloudsdk.appengine.lib.external.datastore import sortable_pb_encoder

if datastore_pbs._CLOUD_DATASTORE_ENABLED:
  from googlecloudsdk.appengine.lib.external.datastore.datastore_pbs import googledatastore
...
# Python types that are forbidden as property values in filters. This is a
# tuple, instead of a set, so that it can be passed directly into isinstance().
_PROPERTY_TYPES_FORBIDDEN_IN_FILTERS = (Blob, Text, EmbeddedEntity)

# Meanings for "raw" properties that cannot be indexed. This is a tuple, instead
# of a set, so that it can be passed directly into isinstance().
...
def ValidatePropertyKey(name, value):
  ...
  """
  # Key property values generated from an entity value can be incomplete.
