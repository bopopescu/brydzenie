...
"""managed-instance-groups list-operations command."""
...
class ListOperations(base.Command):
  ...
  def Run(self, args):
    """Run 'managed-instance-groups list-operations'.
    ...
    request = client.zoneOperations().list(
        project=project,
        zone=args.zone)
  ...
  def Display(self, args, result):
    ...
    Args:
      ...
      result: a list of dicts, where each dict has information about one
          managed instance group operation
    ...
    if args.l:
      util.PrintTable(result, 'zone-operation')
    else:
      for group in result:
