...
def FindExecutableOnPath(executable, path=None, pathext=None):
  ...
  # If given executable has extension in PATHEXT then allow to match without
  # any additional extensions as a first choice.
  if extension in extensions:
    extensions.insert(0, '')
  for directory in paths:
    exec_path = os.path.join(directory, executable)
    for ext in extensions:
      full = os.path.normpath(exec_path + ext)
